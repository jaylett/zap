...
# Read the source-form of a manual and generate various output forms.
...
#
# Probably re-write in C to get better speed for PS output.
...
sub got_para {
  ...
  while (/\S/) {
    ...
    if (/^(\\[iI])?\\c/) {
      ...
      s/^\\c//;
      # was die ... but that's too drastic
      print "(Potentially) badly formatted \\c: \\c$_\n" if !/{(([^\\}]|\\.)*)}(.*)$/;
      $w = $1;
      $_ = $3;
      $w =~ s/\\{/{/g;
      $w =~ s/\\}/}/g;
...
sub write_txt {
  ...
  # Preamble.
  $title = "Zap: A Programmers' Editor";
...
sub write_html {
  ...
  open TEXT,">Contents.html";
  select TEXT;
  &html_preamble(0);
  print "<p>This manual documents Zap, a Programmers' Editor for Acorn Risc OS.\n";
  print "<p>";
...
#      substr($fname,8 - length $number, length $number) = $number;
  ...
  for ($para = 0; $para <= $#pnames; $para++) {
    ...
    if ($ptype eq "chap") {
      ...
      $title = "Chapter $1: ";
      $xref = $2;
      &html_jumppoints; &html_foot; select STDOUT; close TEXT;
  ...
  select TEXT;
  &html_preamble(0);
  print "<p align=center><a href=\"Contents.html\">Contents</a>\n";
  print "<p>";
  &html_index;
  print "<p align=center><a href=\"Contents.html\">Contents</a>\n";
  &html_foot;
...
# Print out the map ...
  ...
  print "<AREA ALT=\"Zap homepage\" SHAPE=RECT COORDS=\"58,0,102,81\" HREF=\"/\">\n";
  print "</MAP>\n\n";
...
sub html_preamble {
  ...
  print "<html><head><title>Zap Manual</title>\n";
  print "<link rel=stylesheet href=\"/style/main.css\" type=\"text/css\">\n";
  print "<link rel=author href=\"mailto:webmaster\@zap.uk.eu.org\">\n";
  print "<link rel=copyright href=\"/copyright.html\">\n";
  print "<meta name=generator content=\"docsrc.pl by Simon Tatham and James Aylett\"></head>\n";
  print "<body background=\"/img/background.gif\" text=\"#000000\" bgcolor=\"#dcdcdc\" link=\"#20209f\" vlink=\"#008000\" alink=\"#ff2020\">\n";
  print "<center><img alt=\"Site map\" SRC=\"/img/zapbar.gif\" border=0 usemap=\"#docmap\" width=600 height=82></center>\n\n";
  print "<h1>Zap documentation</h1>\n\n";
...
#sja: NOBR doesn't exist!    return $pfx . "<code><nobr>${w}</nobr></code>" . $sfx;
# for the moment, inline code will split (looks nice-ish anyway)
